...
@section('content')
   <!-- Hero -->
        ...
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill h3 my-2">
                    Edit Product <small class="d-block d-sm-inline-block mt-2 mt-sm-0 font-size-base font-w400 text-muted"></small>
                </h1>
    ...
    <div class="content">
		...
		<form action="{{route('updateproduct', $product->id)}}" method="POST" class="mb-5" enctype="multipart/form-data">
			@csrf
            {{ method_field('patch')}}
			<div class="form-group">
				<label for="title">Product Name</label>
				<input type="text" name="name" class="form-control {{$errors->has('name')? 'is-invalid': ''}}" value="{{$product->name}}" placeholder="Name">
			</div>
			
			<div class="form-group">
				<label for="description">Product Description</label>
				<textarea type="text" id="article-ckeditor" name="description" placeholder="Description" rows="5" class="form-control {{$errors->has('description')?'is-invalid': ''}}" >{{$product->description}}</textarea>
			</div>
			<div class="form-group">
...
<script type="text/javascript">
	...
	function doSomething(e) {
		e.preventDefault();
		const url = "{{route('deleteImage', $product->id)}}";
		const findMatch = (el) => {

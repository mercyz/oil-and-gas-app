  ...
  <!-- Main SEction -->
    <section class="main-body padding-top">
        <div class="container padding-bottom">
            <div class="row">
                <div class="col-md-8">
                    <h2 class="bg-title">Products</h2>
                    <div class="row">
                    	@foreach($products as $product)
                        <div class="col-md-6">
                            <div class="card blog-card mb-3">
                                <img src="uploads/products/{{$product->featuredImage}}" class="card-img-top" alt="">
                                <div class="card-body">
                                    <h5 class="card-title"><a href="{{route('showProduct', $product->id)}}">{{$product->name}}</a></h5>
                                    <p class="card-text">{{ strip_tags(str_limit($product->description , 100, '...'))}}</p>
                                    <a href="{{route('showService', $product->id)}}" class="read-more">Learn More <i class="fa fa-angle-double-rightgit"></i></a>
                                </div>
                            </div>
                        </div>
                        @endforeach
